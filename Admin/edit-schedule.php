...
$stmt = $conn->prepare("SELECT * FROM classes_schedule WHERE id=?");
...
if (isset($_POST['submit'])) {
    ...
    if (empty($class_name) || empty($trainer) || empty($start) || empty($end) || empty($day) || empty($room)) {
        // Handle validation errors
        die("Every field MUST be filled");
    }

// Prepare an UPDATE statement to update the user record
$stmt = $conn->prepare("UPDATE classes_schedule SET class_name=?, instructor_name=?, start_time=?, end_timd=?, day_of_week=?, room_number=?  WHERE id=?");
$stmt->bind_param("sssssii", $class_name, $trainer, $start, $end, $day, $room, $id);

// Execute the statement and check for errors
if ($stmt->execute()) {
    echo "Record updated successfully";
    header("Location: classes-schedule.php");
} else {
...
<div class="sidebar close">
  ...
  <div class="home-section">
    ...
    <div class="container">
            <div class="title">Edit Schedule Details</div>
            <div class="content">
            <?php
            if(isset($error)){
            ...
            <input type="hidden" name="id" value="<?=$user['id'];?>">
                                            <div class="user-details">
                                                ...
                                                <div class="input-box">
                                                    ...
                                                    <input type="text" name="start_time" value="<?=$user['start_time'];?>" class="form-control" required>
                                                </div>

                                                <div class="input-box">
                                                    <label for="">End Time </label>
                                                    <input type="text" name="end_time" value="<?=$user['end_time'];?>" class="form-control" required>
                                                </div>

                                                <div class="input-box">
                                                    <label for="">Day of Week </label>
                                                    <input type="text" name="day_of_week" value="<?=$user['day_of_week'];?>" class="form-control" required>
                                                </div>

                                                <div class="input-box">
                                                    <label for="">Room Number </label>
                                                    <input type="text" name="room_number" value="<?=$user['room_number'];?>" class="form-control" required>
                                                </div>
