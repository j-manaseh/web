  ...
  $status = $_POST['paid'];
      ...
      if (!empty($member_id) || !empty($member_fname) || !empty($member_lname) || !empty($email) || !empty($subscription) || !empty($sub_id) || !empty($amount) || !empty($status)) {
         ...
         if (mysqli_connect_error()) {
           ...
           {
             $SELECT = "SELECT invoice_id From invoices Where invoice_id = ? Limit 1";
             $INSERT = "INSERT Into invoices (id, fname, lname, email, plan_name, sub_id, amount, paid) values(?, ?, ?, ?, ?, ?, ?, ?)";
             ...
             if ($rnum==0) {
                 ...
                 $stmt->bind_param("issssiss", $member_id, $member_fname, $member_lname, $email, $subscription, $sub_id, $amount, $status);
                 $stmt->execute();
                 ?>
           ...
           echo '<div style="padding:20px 0px;" role="alert">
            ...
            <form method="POST">
                                            <div class="user-details">
                                                ...
                                                <div class="button">
                                                    <button name="add_invoice" type="submit">Create Invoice</button>
                                                </div>
                                            </div>
            </form>
            </div>
